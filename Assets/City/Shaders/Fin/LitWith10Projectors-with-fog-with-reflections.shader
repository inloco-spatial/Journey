Shader "Custom/LitWith10Projectors_CleanFinal_LitSimple_with_fog_and_reflection"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            TEXTURECUBE(_ReflectionProbeTexture);
            SAMPLER(sampler_ReflectionProbeTexture);

            float4 _ReflectionProbeParams; // x = intensity
...
half3 reflectionColor = SAMPLE_TEXTURECUBE(_ReflectionProbeTexture, sampler_ReflectionProbeTexture, reflectionDir).rgb;

// Fresnel эффект дл€ м€гкого отражени€
float fresnel = pow(1.0 - saturate(dot(viewDir, normalWS)), 5.0);
reflectionColor *= fresnel;
    // Final color
    half3 finalColor = baseColor.rgb * lighting + proj;

// —мешиваем отражение с финальным цветом
finalColor = lerp(finalColor, reflectionColor, _ReflectionStrength * _ReflectionProbeParams.x); // 0.2 Ч сила отражени€
