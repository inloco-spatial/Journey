Shader "Custom/LitWith10Projectors_CleanFinal_LitSimple_with_fog_2side"
{
    Properties
    {
...
half4 frag(Varyings IN) : SV_Target
{
    ...
    half ao = tex2D(_AOMap, IN.uv).r; // обычно AO хранится в красном канале
ao = lerp(1.0, ao, _AOStrength); // регулируем силу AO

    half3 normalTS = UnpackNormal(tex2D(_BumpMap, IN.uv));
    normalTS.xy *= _NormalStrength; // ← усиливаем или ослабляем нормали
    normalTS = normalize(normalTS); // обязательно нормализовать
    ...
    half3 lighting = (bakedGI + mainLightColor + additionalLightsColor) * ao;
